...
import indi.huishi.sms.service.UserService;
import indi.huishi.sms.util.CodeUtil;
...
//    @Resource(name = "userServiceRedis")
    ...
    @GetMapping(value = "/unLoginError")
    public String toErrorPage(){
        return "error/3xx";
    }

    @RequestMapping("/over/{do}")
    public String doOver(@PathVariable("do")String done){
        log.info("{}已完成", done);
        return "mail/2xx";
    }

    @RequestMapping("/fail/{do}")
    public String doFail(@PathVariable("do")String done){
        log.info("{}失败", done);
        return "mail/5xx";
    }
...
//            restTemplate.getForObject()
            return "redirect:http://localhost:8081/student/show.action?pageNo=1";
        }
    }
    ...
    protected String logout(HttpSession session){
        ...
        session.invalidate();
        //2.重定向到首页
        return "redirect:http://localhost:8080/user/login";//带端口号，防止学生页面无法返回
    }
