...
import indi.huishi.sms.pojo.Statistics;
import indi.huishi.sms.pojo.Student;
import indi.huishi.sms.service.StudentService;
import indi.huishi.sms.util.BeanUtil;
...
//        System.out.println("updateById"+student);
    ...
    public String stat(Model model, HttpSession session) throws Exception {
        List<Statistics> statistics = studentService.statistics();
        session.setAttribute("statistics", statistics);// 将统计信息保存到session
        // 用于柱状图
    ...
    public String jumpTo(Model model, @PathVariable("sourceName") String sourceName){
        return "menu/" + sourceName + ".html";
    }

//
    @RequestMapping("/sendmail.action")
    public String sendMail() throws UnsupportedEncodingException, MessagingException {
        return "redirect:http://localhost:8025/mail/send.action";
    }

    @RequestMapping("/logout.action")
    protected String logout(HttpSession session){
        return "redirect:http://localhost:8080/user/logout.action";//带端口号，防止学生页面无法返回
    }
